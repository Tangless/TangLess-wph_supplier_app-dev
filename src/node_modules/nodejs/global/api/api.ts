...
export function GetDemandList(rootRequest: potato.RootRequest, data: { prov: string }, success?: (data: demand.DemandInfo[]) => void, fail?: (error: potato.PError) => void): Promise<demand.DemandInfo[]> {
    ...
    // //记录项目列表分页,查询cookie中有没有项目id记录
    // let demandLists_id = cookies[ApiDomain + '$demandLists_id'];
    // if (!demandLists_id) {
    //     //不存在id
    //     Object.assign(data, { sort: "id", order: "desc", cnt: 20 });
    // }else{
    //     Object.assign(data, { sort: "id", order: "asc", cnt: 20,"sort_val": demandLists_id});
    // }
    Object.assign(data, { sort: "id", order: "desc", cnt: 20 })
    return potato.callApi({
