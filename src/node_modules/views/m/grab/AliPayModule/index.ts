...
class VPresenter extends project.VPresenter {
    ...
    public setCtx = function (chargeBill) {
        ...
        //充值时的订单号
        this.alipay_code = chargeBill.bill.id;
        this.alipay_info = chargeBill.alipay;
    };
    //抢单页充值时点击
    _evt_doCharge = function () {
        var that = this;
        var alipay_code = that.alipay_code;
        var alipay_info = that.alipay_info;
        ...
        // alert("afterpayto::"+https%3A%2F%2Fapp.wanpinghui.com%2Fuser%2FValueAddService%3FfromAlipay);
        var url = "`{APIURL}`/Bill/pay?bill_id=" + alipay_code + '&return_url=' + afterpayto;
        // alert("url::"+url);
        // window.open(url);

        var setToNative = {
            "url": "`{APIURL}`/Bill/pay?bill_id=" + alipay_code,
            "alipay_code":alipay_info,
        };
        var sendMessage = JSON.stringify(setToNative);
        //将参数传给native
        if (funs.isAndroid()) {
            //android
            alert(JSON.stringify(setToNative))
            window.android_info.androidOpenAlipay(sendMessage);
        } else {
            //ios 
            alert(JSON.stringify(setToNative))
            window.webkit.messageHandlers.iosOpenAlipay.postMessage(sendMessage);
        }
        /////
