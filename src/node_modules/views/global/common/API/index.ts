...
// 登录
export function Login(mobile: string, sms_captcha: string, push_reg_id: string, success?: (data: boolean) => void, fail?: (error: Error) => void): Promise<boolean> {
    return ptcache.load({
        url: apiServer + '/Login/handleLogin', data: { mobile: mobile, sms_captcha: sms_captcha, push_reg_id: push_reg_id }, method: "post", render: function (data) {
            if (!(data instanceof Error)) {
...
// 获取项目列表(翻页用)
export function GetDemandList(data: { sort_val: string, province?: string, type?: string, sort?: string, pagesize?: number, order?: string }, success?: (data: any) => void, fail?: (error: Error) => void): Promise<any> {
    data = $.extend({}, data, { pagesize: 20, sort: "id", prov: model.globalData.selected_province.province_id, order: "asc" });
    return ptcache.load({
...
/**通过手机号查询用户信息 */
export function GetInfoByMobile(mobile: string, success?: (data: any) => void, fail?: (error: Error) => void): Promise<boolean> {
    return ptcache.load({
        url: apiServer + '/User/getInfoByMobile', data: { mobile: mobile }, method: "post", render: function (data) {
            if (!(data instanceof Error)) {
                if (data.status != 200) {
                    return new tomato.PError(data.status, data.info);
                }
...
/**投票 */
export function Vote(data: any, success?: (data: any) => void, fail?: (error: Error) => void): Promise<any> {
    return ptcache.load({
        url: apiServer + '/Supplier/Demand/vote', data: data, method: "post", render: function (data) {
            if (!(data instanceof Error)) {
                if (data.status != 200) {
                    return new tomato.PError(data.status, data.info);
                }
                data = mappingDemand(data.demand_info);
...
/**取消投票 */
export function Unvote(data: any, success?: (data: any) => void, fail?: (error: Error) => void): Promise<any> {
    return ptcache.load({
        url: apiServer + '/Supplier/Demand/unvote', data: data, method: "post", render: function (data) {
            if (!(data instanceof Error)) {
                if (data.status != 200) {
                    return new tomato.PError(data.status, data.info);
                }
